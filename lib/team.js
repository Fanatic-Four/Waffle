...
var team = [
	...
	member('tdnguyen', 'Tommy', 'Nguyen', 'juinor'),
	member('slambrou', 'Steven', 'Lambrou', 'senior'),
	member('jinhuiliang', 'Evans', 'Liang', 'juinor'),
	member('mccaffre', 'Joe', 'McCaffrey', 'senior'),
	member('sorloff', 'Samuel', 'Orloff', 'juinor'),
 ...
 */
function copy(thisMember) {
	//Changed parameter of this function to a different name than member so I can call the member() function defined above
	return member(
		thisMember.user,
		thisMember.fname,
		thisMember.lname,
...
function find(user) {
	...
	for (var teamIdx = 0; teamIdx < team.length; teamIdx++) {
		if (team[teamIdx].user === user) {
			return copy(team[teamIdx]); //Only returns a copy of the member object when the user string is a match
		}
	}
	return null;
}
 ...
 */
function one(user) {
	// Check for type of user is string
	if (typeof user !== 'string') {
		return result(false, "user is not of type string!", []);
	}

	//Look for user
	var student = find(user);
	if (student !== null) {
		return result(true, "team member found", [student]);
	}
